...
{
	public partial class KenticoResourceSearch : BasePage
	{
		public string ImageWebFolder { get; set; }
		protected bool _Perm_Resource_Link_Ok;
		protected IEnumerable<string> NodeAliasPaths;
		...
		{
			radGridResults.DataSource = SearchResourcesWithCriteria(SessionObject.LoggedInUser, Master.CurrentCriteria(), NodeAliasPaths);
			radGridResults.DataBind();
		}

		public static List<Thinkgate.Base.Classes.Resource> SearchResourcesWithCriteria(ThinkgateUser user, CriteriaController criteriaController, IEnumerable<string> nodeAliasPaths)
		{
			...
			return Thinkgate.Base.Classes.Resource.SearchAdvancedKentico(selectedCategories, selectedTypes, selectedSubTypes, resourceName, searchText, searchOption,
				associationType, associationField1, associationField2, associationField3, selectedStandards, nodeAliasPaths);
		}
		...
		protected void radGridResults_ItemDataBound(object sender, GridItemEventArgs e)
		{
			if (e.Item is GridDataItem)
			{
				...
				var hlView = (System.Web.UI.HtmlControls.HtmlAnchor)item.FindControl("hlView");
				...
				if (itemResource.DocumentType == Base.Enums.ResourceType.WebLink.ToString()) 
				{
					hlView.HRef = itemResource.ViewLink;
				}
				else if (itemResource.DocumentType == Base.Enums.ResourceType.Attachment.ToString() && !string.IsNullOrEmpty(itemResource.ViewLink))
				{
					var baseUri = new Uri(ConfigurationManager.AppSettings["KenticoServerUrl"]);
					var completeUri = new Uri(baseUri, KenticoHelper.KenticoVirtualFolder + "/CMSPages/getfile.aspx");
					hlView.HRef = completeUri.ToString() + "?guid=" + itemResource.ViewLink + "&disposition=inline";
				}
